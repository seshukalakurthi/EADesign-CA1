...
app.listen(PORT, () => {
  console.log(JSON.stringify({ event: 'start', port: PORT, svc: 'checkout' }));
...
async function initDb() {
  try {
    ...
    `);
    console.log(JSON.stringify({ event: 'db_init_ok', svc: 'checkout' }));
  } catch (err) {
...
app.post('/checkout', async (req, res) => {
  ...
  try {
    const [pRes, iRes] = await Promise.all([
      fetchWithTimeout(PRICING_URL, { method: 'POST', headers, body: JSON.stringify({ subtotal }) }, DEP_TIMEOUT_MS),
      fetchWithTimeout(`${INVENTORY_URL}/${encodeURIComponent(sku)}`, { method: 'GET', headers }, DEP_TIMEOUT_MS),
    ]);
    ...
    if (!iRes.ok) {
      ...
      return res.status(iRes.status === 404 ? 404 : 502).json({ error: 'inventory service error', detail: err, rid });
    }
    ...
    return res.status(isTimeout ? 504 : 502).json({
      error: isTimeout ? 'dependency timeout' : 'dependency unavailable', rid,
    });
  }
  ...
  return res.json({ rid, sku, qty, pricing: pricingResult, inventory: inventoryResult, total, status: 'completed' });
});